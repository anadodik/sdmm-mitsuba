...
OUT_PATH = os.path.join(RESULTS_PATH, 'thomas_results')
...
def copy(from_paths):
    ...
    for root, _, files in os.walk(top):
        ...
        if match and match.group(1) in SCENES:
            ...
            for f in files:
                if not f.startswith('iteration') and types_pattern.match(f):
...
SDMM_FROM_PATHS = [
    RESULTS_PATH + '/(.*)/sdmm/radiance/maxDepth=10,rrDepth=10,sampleCo=1024,samplesP=8',
    RESULTS_PATH + '/(.*)/sdmm/product/maxDepth=10,rrDepth=10,sampleCo=1024,samplePr=true,samplesP=8',
]
