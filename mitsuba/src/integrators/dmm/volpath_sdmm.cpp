...
public:
    ...
    void optimize_async_run() {
        ...
        m_thread_pool->parallelFor(0, (int) m_node_idcs.size(), [&](int i){
            size_t context_i = m_node_idcs[(size_t) i];
            auto& context = nodes[context_i].value;
            std::swap(context->data.normal, context->training_data.normal);
            std::swap(context->data.weight, context->training_data.weight);
            std::swap(context->data.size, context->training_data.size);
            for(int point_i = 0; point_i < context->training_data.size; ++point_i) {
                auto slice = enoki::slice(context->data.point, point_i);
                sdmm::linalg::Vector<float, 3> direction{
                    slice.coeff(3), slice.coeff(4), slice.coeff(5)
                };
