...
#include "../pmc/jmm/nlohmann/json.hpp"
...
public:
    ...
    bool renderSPP(Scene *scene, RenderQueue *queue, const RenderJob *job,
        ...
        while (result && m_passesRendered < nPasses) {
            ...
            auto renderDuration = std::chrono::duration_cast<std::chrono::milliseconds>(iterationEndTime - renderStartTime);

            stats.push_back({
                {"iteration", m_iter},
                {"spp", m_passesRenderedThisIter * m_sppPerPass},
                {"total_spp", m_passesRendered * m_sppPerPass},
                {"elapsed_seconds", (float)iterationDuration.count() / 1000},
                {"total_elapsed_seconds", (float)renderDuration.count() / 1000},
                {"mean_pixel_variance", variance},
                {"ttuv", variance * (float)iterationDuration.count() / 1000},
            });
    ...
    bool render(Scene *scene, RenderQueue *queue, const RenderJob *job,
        ...
        //output the results of each iteration individually
        for (size_t i = 0; i < m_images.size(); ++i) {
            ref<ImageBlock> tmp = new ImageBlock(Bitmap::ESpectrum, film->getCropSize());
            m_images[i]->convert(tmp->getBitmap());
            fs::path hdrFilename = scene->getDestinationFile().parent_path() / fs::path(formatString("iteration_%d.exr", i));;
            ref<FileStream> targetFileHDR = new FileStream(hdrFilename, FileStream::ETruncReadWrite);
            tmp->getBitmap()->write(Bitmap::EOpenEXR, targetFileHDR, 1);
        }
